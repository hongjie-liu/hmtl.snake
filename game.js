...
const gridSize = 16;              
const tileCount = 50;            
canvas.width = gridSize * tileCount;//计算宽度
canvas.height = gridSize * tileCount;//计算长度
let snake = [{ x: 10, y: 10 }];
let stones = [];
let stoneTimer = null;
let food = { x: 15, y: 15 };
...
function draw() {
    ...
    // 画蛇身
    snake.forEach((seg, i) => {
        const alpha = 1 - i * 0.02;//随着索引的增加蛇身颜色变浅
        ctx.fillStyle = `rgba(76, 175, 80, ${alpha})`;//绿色

        ctx.beginPath();//与下面的fill中间绘图
        ctx.roundRect(
            seg.x * gridSize + 2,
            seg.y * gridSize + 2,
            gridSize - 4,
...
stones.forEach(s => {
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(bx + 1, by +1+ size / 2);
    ctx.lineTo(bx + size + 1, by + 1+size / 2);
    ctx.moveTo(bx + 1+size / 2, by + 1);
    ctx.lineTo(bx +1+size / 2, by +1+ size + 1);
    ctx.stroke();
...
}, { passive: true });//优化型选项，监听器不会调用preventDefluat()来阻止默认行为
